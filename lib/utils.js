...
exports.runKubectlProxies = env => {
  exec(
    `kubectl port-forward --namespace=${env} $(kubectl get pods --namespace ${env} -l "app=rabbitmq-${env}-rabbitmq" -o jsonpath="{.items[0].metadata.name}") 5672:5672`
  )
  exec(
    `kubectl port-forward --namespace=${env} $(kubectl get pods --namespace ${env} -l "app=mongodb" -o jsonpath="{.items[0].metadata.name}") 27017:27017`
  )
  exec(
    `kubectl port-forward --namespace=${env} $(kubectl get pods --namespace ${env} -l "app=redis-${env}-redis" -o jsonpath="{.items[0].metadata.name}") 6379:6379`
  )
}
